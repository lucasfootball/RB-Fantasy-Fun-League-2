...
print("\n== ODDS + CHALK ==")
check("odds parsed as ints", str(legs["odds"].dtype) == "Int64" and int(legs["odds"].notna().sum()) == 40)
check("implied prob of -200 is ~0.667", abs(logic.implied_prob(-200) - 2/3) < 1e-9)
check("implied prob of +150 is 0.40", abs(logic.implied_prob(150) - 0.40) < 1e-9)
check("decimal of +100 is 2.0", abs(logic.decimal_odds(100) - 2.0) < 1e-9)
check("fmt_odds signs", logic.fmt_odds(150) == "+150" and logic.fmt_odds(-110) == "-110")
check("week payout is positive and priced 10/10", latest["payout_10"] > 10 and latest["legs_priced"] == 10)

chalk = logic.chalk_ranking(legs, members)
print(chalk.assign(**{"Avg implied": (chalk["Avg implied"]*100).round(1)}).to_string(index=False))
check("Ben is chalkiest", chalk.iloc[0]["Member"] == "Ben")
check("Jacob is biggest gambler", chalk.iloc[-1]["Member"] == "Jacob")
check("chalk covers all 10", len(chalk) == 10)

print("\n== EMPTY DATA (fresh start) ==")
import tempfile
_t = tempfile.NamedTemporaryFile("w", suffix=".csv", delete=False)
_t.write("week,date,funder,submitter,leg,bet_type,odds,result,outcome\n"); _t.close()
empty = logic.load_legs(_t.name)
